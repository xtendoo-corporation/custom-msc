...
class DaruclimeFSMStage(models.Model):
    ...
    code = fields.Char(
        string="Código", required=True, help="Código único para identificar la etapa"
    )
    ...
    order_count = fields.Integer(
        string="Número de Órdenes", compute="_compute_order_count"
    )
    ...
    def _compute_order_count(self):
        ...
        for record in self:
            orders = self.env["msc.fsm.order"].search([("stage_id", "=", record.id)])
        ...
        for vals in vals_list:
            ...
            if vals.get("is_default"):
                ...
                if existing:
                    ...
                    existing.flush_recordset(["is_default"])
        ...
        for record in self:
            if not re.match(r"^[a-zA-Z0-9_]+$", record.code):
                raise ValidationError(
                    _("El código debe contener solo letras, números y guiones bajos.")
                )
