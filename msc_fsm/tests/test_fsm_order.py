...
class TestFSMOrder(TransactionCase):
    ...
    def setUp(self):
        ...
        # Create test tag
        self.tag = self.env["msc.fsm.tag"].create(
            {
                "name": "Test Tag",
                "color": 1,
            }
    ...
    def test_fsm_order_sequence(self):
        ...
        order1 = self.env["msc.fsm.order"].create(
            {
                "partner_id": self.partner.id,
        ...
        order2 = self.env["msc.fsm.order"].create(
            {
                "partner_id": self.partner.id,
    ...
    def test_sale_order_creation(self):
        """Test sale order creation from FSM order"""
        order = self.env["msc.fsm.order"].create(
            {
                "partner_id": self.partner.id,
                "description": "Test sale order creation",
            }
        )

        self.assertFalse(order.sale_order_ids)
        self.assertEqual(order.sale_count, 0)

        # Create sale order
        result = order.action_create_sale_order()

        self.assertTrue(order.sale_order_ids)
        self.assertEqual(result["res_model"], "sale.order")
        self.assertEqual(result["res_id"], order.sale_order_ids[0].id)

    def test_stage_expansion(self):
        """Test stage expansion for kanban view"""
        order_model = self.env["msc.fsm.order"]
